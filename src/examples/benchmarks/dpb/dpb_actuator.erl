...
%%%----------------------------------------------------------------------------
%%% @doc Double pole balancing actuator for benchmarking purposes.
%%% @end
%%%----------------------------------------------------------------------------
-module(dpb_actuator).
...
%%      Pole Balancing simulator, and waits for the resulting Fitness and
...
actuate(dpb, {AgentId, Output, Params, _VL, ScapePid, _ActuatorId, ModState}) ->
  private_scape:actuate(ScapePid, AgentId, self(), push, {Params, Output}),
  ModState.