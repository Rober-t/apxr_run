...
-ifdef(TEST).
-export([do_decode_metadata/1, gzip/1]).
-endif.
-define(VERSION, <<"3">>).
...
%%     {ok, {Tarball, Checksum}} = apxr_tarball:create(Metadata, Files).
%%     Tarball.
%%     %%=> <<86,69,...>>
%%     Checksum.
%%     %%=> <<40,32,...>>
...
%% @doc
%% Unpacks a project tarball.
%%
%% Examples:
%%
%% ```
%%     apxr_tarball:unpack(Tarball, memory).
%%     %%=> {ok,#{checksum => <<...>>,
%%     %%=>       contents => [{"src/foo.erl",<<"-module(foo).">>}],
%%     %%=>       metadata => #{<<"name">> => <<"foo">>, ...}}}
%%
%%     apxr_tarball:unpack(Tarball, "path/to/unpack").
%%     %%=> {ok,#{checksum => <<...>>,
%%     %%=>       metadata => #{<<"name">> => <<"foo">>, ...}}}
%% '''
...
unpack(Tarball, Output) ->
	case apxr_erl_tar:extract({binary, Tarball}, [memory]) of
		{ok, []} ->
			{error, {tarball, empty}};
...
format_error({tarball, Reason}) -> "tarball error, " ++ apxr_erl_tar:format_error(Reason);
format_error({inner_tarball, Reason}) -> "inner tarball error, " ++ apxr_erl_tar:format_error(Reason);
format_error({metadata, invalid_terms}) -> "error reading project metadata: invalid terms";
format_error({metadata, not_key_value}) -> "error reading project metadata: not in key-value format";
format_error({metadata, Reason}) -> "error reading project metadata" ++ safe_erl_term:format_error(Reason);
...
normalize_metadata(Metadata1) ->
	Metadata2 = maybe_update_with(<<"links">>, fun try_into_map/1, Metadata1),
	Metadata3 = maybe_update_with(<<"extra">>, fun try_into_map/1, Metadata2),
	guess_build_tools(Metadata3).

guess_build_tools(#{<<"build_tool">> := _BuildTools} = Metadata) ->
	Metadata;
guess_build_tools(#{<<"files">> := Filenames} = Metadata) ->
	...
	Metadata#{<<"build_tool">> => lists:last(BuildTools)};
...
unpack_tarball(ContentsBinary, memory) ->
	apxr_erl_tar:extract({binary, ContentsBinary}, [memory, compressed]);
...
create_memory_tarball(Files) ->
	Path = tmp_path(),
	{ok, Tar} = apxr_erl_tar:open(Path, [write]),

	try
		add_files(Tar, Files)
	after
		ok = apxr_erl_tar:close(Tar)
	end,
...
add_file(Tar, {Filename, Contents}) when is_list(Filename) and is_binary(Contents) ->
	ok = apxr_erl_tar:add(Tar, Contents, Filename, tar_opts());
...
add_file(Tar, {Filename, AbsFilename}) when is_list(Filename), is_list(AbsFilename) ->
	...
	case FileInfo#file_info.type of
		symlink ->
			ok = apxr_erl_tar:add(Tar, {Filename, AbsFilename}, tar_opts());
		directory ->
			case file:list_dir(AbsFilename) of
				{ok, []} ->
					apxr_erl_tar:add(Tar, {Filename, AbsFilename}, tar_opts());
		...
		_ ->
			...
			ok = apxr_erl_tar:add(Tar, Contents, Filename, Mode, tar_opts())
	end.
...
decode_base16(Base16) ->
	<< <<(unapxr(H) bsl 4 + unapxr(L))>> || <<H,L>> <= Base16 >>.

unapxr(D) when $0 =< D andalso D =< $9 ->
	D - $0;
unapxr(D) when $a =< D andalso D =< $f ->
	10 + D - $a;
unapxr(D) when $A =< D andalso D =< $F ->
	10 + D - $A.