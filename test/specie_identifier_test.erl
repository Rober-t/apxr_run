-module(specie_identifier_test).

-include_lib("eunit/include/eunit.hrl").

%% macros

-define(CXID, {cortex, {origin, 38.7377260318}}).
-define(AGENT_ID, {neuron, {0.0, 0.656234565}}).

%% runners

specie_identifier_test_() ->
  {setup,
    fun setup/0,
    fun cleanup/1,
  [
...
tot_n_subtest() ->
  ?assertEqual(5, specie_identifier:tot_n(?AGENT_ID)).

%% helpers

setup() ->
  SpecieId = {specie, 0.3535435345},
  ...
  Pattern = [{rand:uniform(), [NId]} || NId <- NIds],
